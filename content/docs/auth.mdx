...
| `GOOGLE_CLIENT_ID` | Optional | Google OAuth client ID |
| `GOOGLE_CLIENT_SECRET` | Optional | Google OAuth client secret |
| `GOOGLE_REDIRECT_URI` | Optional | `http://localhost:5173/api/auth/google/callback` |

<Callout type="info">
In development, the Vite plugin runs API routes at the same origin (`http://localhost:5173`) — no separate backend server needed!
</Callout>

</ForFramework>

<ForFramework value="tanstack-start">
...
| `GOOGLE_CLIENT_ID` | Optional | Google OAuth client ID |
| `GOOGLE_CLIENT_SECRET` | Optional | Google OAuth client secret |
| `GOOGLE_REDIRECT_URI` | Optional | `http://localhost:3000/api/auth/google/callback` |
...
### Configure Vite (Vite only)

<FrameworkContent>
<ForFramework value="vite">

The Vite setup includes a plugin that handles API routes during development. Add the plugin to your `vite.config.ts`:
...
export default defineConfig({
  plugins: [
    react(),
    authApiRoutesPlugin(), // Add this line
  ],
  // Your other config...
})
```

This enables API routes to run alongside your frontend during `npm run dev`. No separate backend server needed in development!

</ForFramework>
<ForFramework value={["nextjs", "tanstack-start"]}>
{/* No additional config needed for Next.js or TanStack Start */}
</ForFramework>
</FrameworkContent>
...
<FrameworkContent>
<ForFramework value="nextjs">`http://localhost:3000/api/auth/login/google/callback`</ForFramework>
<ForFramework value={["vite", "tanstack-start"]}>`http://localhost:3000/api/auth/google/callback`</ForFramework>
</FrameworkContent>
