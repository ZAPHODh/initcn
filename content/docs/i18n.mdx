...
- **Type-safe translations** - Full TypeScript autocomplete for translation keys
- **Server & Client support** - Works seamlessly with React Server Components and Client Components
- **Namespace organization** - Organize translations by feature/domain
- **Zero dependencies** - No additional UI components required
- **Automatic routing** - Middleware handles locale detection and URL rewriting
- **SEO-friendly** - Generates proper metadata for each locale
...
<Tabs items={["npm", "pnpm", "yarn", "bun"]}>
  <Tab value="npm">
  ```bash
  npx shadcn@latest add https://initcn.vercel.app/r/i18n-next-international.json
  ```
  </Tab>

  <Tab value="pnpm">
  ```bash
  pnpm dlx shadcn@latest add https://initcn.vercel.app/r/i18n-next-international.json
  ```
  </Tab>

  <Tab value="yarn">
  ```bash
  yarn dlx shadcn@latest add https://initcn.vercel.app/r/i18n-next-international.json
  ```
  </Tab>

  <Tab value="bun">
  ```bash
  bunx --bun shadcn@latest add https://initcn.vercel.app/r/i18n-next-international.json
  ```
  </Tab>
</Tabs>

This installs:
- **Locale configuration** - Client and server i18n setup
- **Example translations** - English translations with "not found" namespace
- **404 page** - Localized catch-all page
- **Language switcher** - Simple component for changing locales
...
app/
├── layout.tsx          # Keep at root
├── globals.css         # Keep at root
├── api/                # Keep at root
├── page.tsx            # Move to [locale]/page.tsx
├── dashboard/          # Move to [locale]/dashboard/
└── about/              # Move to [locale]/about/
```
...
└── [locale]/           # All app routes go here
    ├── page.tsx        # Homepage
    ├── dashboard/      # Dashboard routes
    ├── about/          # About page
    └── [...rest]/      # 404 catch-all (installed by registry)
        └── page.tsx
```

### Update root layout

Add locale parameter support to your root layout:
...
├── en.ts                  # Main English translations
├── pt.ts                  # Main Portuguese translations
├── not-found/             # 404 pages
│   ├── en.ts
│   └── pt.ts
├── auth/                  # Authentication
