...
<Tabs items={["npm", "pnpm", "yarn", "bun"]}>
  <Tab value="npm">
  ```bash
  npx shadcn@latest add @initcn/subscription
  ```
  </Tab>

  <Tab value="pnpm">
  ```bash
  pnpm dlx shadcn@latest add @initcn/subscription
  ```
  </Tab>

  <Tab value="yarn">
  ```bash
  yarn dlx shadcn@latest add @initcn/subscription
  ```
  </Tab>

  <Tab value="bun">
  ```bash
  bunx --bun shadcn@latest add @initcn/subscription
  ```
  </Tab>
</Tabs>

This installs:
- **payment-stripe-shared** (auto-installed) - Stripe utilities, pricing, config, types
- **payment-stripe-prisma** - Database operations, webhooks, checkout, billing portal
...
An `.env.example.subscription` file was installed with the registry. Append it to your `.env.local`:

```bash
cat .env.example.subscription >> .env.local
```

Or add these manually to your `.env.local`:
...
You can now create checkout sessions and manage subscriptions. The system includes:

- API route at `/api/stripe` (checkout & billing portal)
- Webhook handler at `/api/webhooks/stripe`
- Server actions for checkout and billing management
