...
export function InstallCommand({ feature, className }: InstallCommandProps) {
  ...
  const variant = isLoaded ? VARIANTS[feature][config.orm] : VARIANTS[feature].prisma;

  if (!variant) {
    return (
      <div className={cn("rounded-lg border bg-fd-card p-4", className)}>
        <p className="text-fd-muted-foreground text-sm">
          This configuration is coming soon for the selected ORM.
        </p>
      </div>
    );
  }

  const command = `${PM_COMMANDS[activePm]} shadcn@latest add @initcn/${variant}`;
  ...
  return (
    <div className={cn("rounded-lg border bg-fd-card overflow-hidden", className)}>
